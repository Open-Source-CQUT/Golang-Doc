...
现在要进行编译的话，就不能单单指定`main.go`一个文件了，要指定当前文件夹，如下
...
一般都是go调用c，很少会有c调用go的情况。如果要导出go函数，只需加上`//export func_name`注释，并且其参数和返回值都得是cgo支持的类型，例子如下
...
导出的go函数返回值不能带指针，在这种情况下cgo没法判断其是否被引用，也不好固定内存，如果返回的内存被引用了，然后在go中这段内存被GC掉了或者发生偏移，将导致指针越界，如下所示。
...
它有两种检查级别，可以设为`1`，`2`，级别越高检查造成运行时开销越大。 
...
上述这几种字符串传递方法涉及到了一次内存拷贝，在传递过后实际上是在c内存和go内存中各自保存了一份，这样做会更安全，因为c如果直接引用了go的内存，保不准什么时候就被GC掉了，c是无法得知这种情况的。话虽如此，我们依然可以直接传递指针给c函数，甚至可以在c中直接修改go中的字符串，看下面的例子
...
例子通过`unsafe.SliceData`直接获取了字符串底层数组的指针，并将其转换为了c指针后传递给c函数，该字符串的内存是由go进行管理的，自然也就不再需要free，这样做的好处就是传递的过程不再需要拷贝，但有一定的风险，原因上面已经说过了。下面的例子演示了在c中修改go中的字符串
...
c中的数组到了go中，其特性也变的跟go数组一样了，有了自己的地址，想要访问数组头部指针，就得对头部元素取址。
...
![](https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202312211707105.png)

![](https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202312211739040.png)

谈到了指针避不开内存，cgo之间相互调用最大的问题就是两门语言的内存模型并不相同，c语言的内存完全是由开发者手动管理，用`malloc()`分配内存，`free()`释放内存，如果不去手动释放，它是绝对不会自己释放掉的，c的内存管理是非常稳定的。而go就不一样了，它带有GC，并且Goroutine的栈空间是会动态调整的，当栈空间不足时会进行增长，那么这样一来，内存地址就可能发生了变化，跟上图一样（图画的并不严谨），这个指针就成了c中常见的悬挂指针。即便cgo在大多数情况可以避免内存移动（由[runtime.Pinner](https://pkg.go.dev/runtime#Pinner)来固定内存），但go官方也不建议在c中长期引用go的内存。但是反过来，go中的指针引用c中的内存的话，就是比较安全的，除非手动调用`C.free()`，否则这块内存是不会被自动释放掉的。

如果要在c和go之间传递指针，就需要先将其转为`unsafe.Pointer`，然后再转换成对于的指针类型，就跟c中的`void*`一样。看两例子，第一个是c指针引用go变量的例子，而且还对变量做了修改。
...
如果没有源代码，或者源代码非常大的时候，直接通过cgo进行代码调用就不太合适了，所以使用动态和静态链接库的方式用go来调用cgo。
...
制作完成后，确保`sum.dll`跟`main.go`是在同级目录，然后在go代码中引入`sum.h`头文件，并且还得通过宏告诉cgo去哪里寻找库文件
...
`CFFLAGS`和`LDFLAGS`这两个都是gcc的编译选项，出于于安全考虑，cgo禁用了一些参数，前往[cgo command](https://pkg.go.dev/cmd/cgo#hdr-Using_cgo_with_the_go_command)了解细节。
