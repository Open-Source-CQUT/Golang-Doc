import{_ as o,V as c,W as l,X as n,Y as s,Z as a,$ as u,a0 as e,F as i}from"./framework-f06be456.js";const d="/images/essential/pprof_1.png",r="/images/essential/pprof_2.png",k="/images/essential/pprof_3.png",v="/images/essential/pprof_4.png",m="/images/essential/pprof_5.png",g="/images/essential/pprof_6.png",b="/images/essential/pprof_7.png",f="/images/essential/pprof_8.png",h="/images/essential/pprof_9.png",x={},y=n("h1",{id:"性能分析",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#性能分析","aria-hidden":"true"},"#"),s(" 性能分析")],-1),w=n("p",null,"当一个程序编写完毕后，我们对它的要求不仅仅只是能运行，还希望它是一个稳定高效的应用。通过各种各样的测试，我们可以保证程序大部分的稳定性，而程序是否高效，就需要我们对其进行性能分析，在此前的内容中，性能分析的唯一手段就只能通过Benchmark来测试某一个功能单元的平均执行耗时，内存分配情况等，然而现实中对程序性能分析的需求远远不止于此，有时候我们需要分析程序整体的CPU占用，内存占用，堆分配情况，协程状态，热点代码路径等等，这是Benchmark所不能满足的。好在go工具链集成了许多性能分析工具以供开发者使用，下面就来逐一讲解。",-1),q=n("h2",{id:"逃逸分析",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#逃逸分析","aria-hidden":"true"},"#"),s(" 逃逸分析")],-1),_=n("p",null,"在go中，变量的内存分配是由编译器决定的，一般就分配到栈上和堆上这两个地方。如果一个本该分配到栈上的变量被分配到了堆上，那么这种情况就称之为逃逸，逃逸分析便是要分析程序中的内存分配情况，由于它是在编译期进行，所以是静态分析的一种。",-1),S={class:"hint-container tip"},P=n("p",{class:"hint-container-title"},"提示",-1),z=e(`<h3 id="引用局部指针" tabindex="-1"><a class="header-anchor" href="#引用局部指针" aria-hidden="true">#</a> 引用局部指针</h3><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">package</span> main
...
<span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>此时打开浏览器访问<code>http://127.0.0.1:8080/debug/pprof</code>，就会出现这样的页面</p><figure><img src="`+d+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>页面中有几个可供选择的选项，它们分别代表了</p><ul><li><code>allocs</code>：内存分配抽样</li><li><code>block</code>：同步原语的阻塞跟踪</li><li><code>cmdline</code>：当前程序的命令行调用</li><li><code>goroutine</code>：跟踪所有的协程</li><li><code>heap</code>：对于存活对象的内存分配抽样</li><li><code>mutex</code>：互斥锁相关信息的跟踪</li><li><code>profile</code>：cpu分析，会分析一段时间并下载一个文件</li><li><code>threadcreate</code>：分析导致创建新OS线程原因</li><li><code>trace</code>：当前程序执行情况的跟踪，同样会下载一个文件</li></ul><p>这里的数据大多数可读性并不高，主要是拿来给工具分析用的，如下所图</p><figure><img src="'+r+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>具体的分析工作要留到后面再进行，除了<code>profile</code>和<code>trace</code>两个选项之外，如果你想要在网页中下载数据文件，可以将<code>query</code>参数<code>debug=1</code>去掉。也可以将这些接口集成到自己的路由中而不是使用默认路由，如下所示</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">package</span> main
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如此一来，也能其集成到其它的web框架中，比如<code>gin</code>，<code>iris</code>等等。</p><h3 id="分析" tabindex="-1"><a class="header-anchor" href="#分析" aria-hidden="true">#</a> 分析</h3><p>在得到了采集的数据文件后，有两种方式进行分析，命令行或网页，两者都需要借助<code>pprof</code>命令行工具，go默认集成该工具，所以不需要额外下载。</p>`,68),C={href:"https://github.com/google/pprof",target:"_blank",rel:"noopener noreferrer"},M=e(`<h4 id="命令行" tabindex="-1"><a class="header-anchor" href="#命令行" aria-hidden="true">#</a> 命令行</h4><p>将此前收集到的数据文件作为参数</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ go tool pprof heap.pb
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,13),$={class:"hint-container tip"},G=n("p",{class:"hint-container-title"},"提示",-1),F={href:"https://github.com/google/pprof/blob/main/doc/README.md",target:"_blank",rel:"noopener noreferrer"},N=e(`<img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202404151646245.png" style="zoom:67%;"><p>网页中总共有6个可查看的项</p><ul><li>Top，同命令top</li><li>Graph，直线图</li><li>Flame Graph，火焰图</li><li>Peek，</li><li>Source，查看源代码</li><li>Disassemble，反汇编查看</li></ul><p>对于内存而言四个维度可以分析</p><ul><li><code>alloc_objects</code>：目前已分配的所有对象数量，包括已释放</li><li><code>alloc_spcae</code>：目前为止已分配的所有内存空间，包括已释放</li><li><code>inuse_objects</code>：正在使用的对象数量</li><li><code>inuse_space</code>：正在使用的内存空间</li></ul><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202404151654998.png" alt="内存分析图" tabindex="0" loading="lazy"><figcaption>内存分析图</figcaption></figure><p>上图最下方的白色叶子节点代表着不同大小的对象占用。</p><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202404151710273.png" alt="cpu分析图" tabindex="0" loading="lazy"><figcaption>cpu分析图</figcaption></figure><p>关于折线图，有几个点要注意</p><ul><li>块的颜色越深，占用越高，线越粗，占用越高</li><li>实线代表着直接调用，虚线代表着略过了一些调用链。</li></ul><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202404151656457.png" alt="内存火焰图" tabindex="0" loading="lazy"><figcaption>内存火焰图</figcaption></figure><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202404151711752.png" alt="cpu火焰图" tabindex="0" loading="lazy"><figcaption>cpu火焰图</figcaption></figure><p>对于火焰图而言，从上往下看是调用链，从左往右看是cum的占用百分比。</p><h2 id="trace" tabindex="-1"><a class="header-anchor" href="#trace" aria-hidden="true">#</a> trace</h2><p>pprof主要负责分析程序的资源占用，而trace更适合跟踪程序的运行细节，它与前者的数据文件互不兼容，由<code>go tool trace</code>命令来完成相关的分析工作。</p><p>如果是手动采集的数据，可以将文件名作为参数</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>$ go tool trace trace.out
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>打开后页面大概如下所示</p><figure><img src="`+v+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>这里面主要包含了以下几个部分，这些数据要看懂还挺不容易的。</p><ul><li><p>Event timelines for running goroutines</p><ul><li><p>trace by proc：显示每一时刻在该处理器上运行的协程时间线</p><figure><img src="'+m+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure></li><li><p>trace by thread：显示每一时刻在OS线程上运行的协程时间线</p><figure><img src="'+g+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure></li><li><p>Goroutine analysis：展示每组主函数的协程相关统计信息</p><figure><img src="'+b+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><figure><img src="'+f+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure></li></ul></li><li><p>Profiles</p><ul><li>Network blocking profile： 因网络IO而阻塞的协程信息</li><li>Synchronization blocking profile：因同步原语而阻塞的协程信息</li><li>Syscall profile：因系统调用而阻塞的协程信息</li></ul></li><li><p>User-defined tasks and regions</p><ul><li>User-defined tasks：用户定义任务的相关协程信息</li><li>User-defined regions：用户定义代码区域的相关协程信息</li></ul></li><li><p>Garbage collection metrics</p><ul><li><p>Minimum mutator utilization：展示最近GC的最大耗时</p><figure><img src="'+h+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure></li></ul></li></ul>',25);function O(L,E){const p=i("RouterLink"),t=i("ExternalLinkIcon");return c(),l("div",null,[y,w,q,_,n("div",S,[P,n("p",null,[s("前往"),a(p,{to:"/essential/impl/8.mem.html"},{default:u(()=>[s("内存分配")]),_:1}),s("文章了解go具体是如分配内存的。")])]),z,n("p",null,[s("pprof开源地址："),n("a",C,[s("google/pprof: pprof is a tool for visualization and analysis of profiling data (github.com)"),a(t)])]),M,n("p",null,[s("我们可以将转换可视化的格式，"),B,s("支持相当多的格式，比如pdf，svg，png，gif等等（需要安装"),n("a",D,[s("Graphviz"),a(t)]),s("）。")]),T,n("div",$,[G,n("p",null,[s("关于如何分析数据，前往"),n("a",F,[s("pprof: How to read the graph"),a(t)]),s("了解更多")])]),N])}const U=o(x,[["render",O],["__file","140.pprof.html.vue"]]);export{U as default};
