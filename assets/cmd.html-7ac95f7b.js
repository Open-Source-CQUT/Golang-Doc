import{_ as t,V as c,W as d,X as n,Y as s,Z as a,$ as p,a0 as e,F as l}from"./framework-f06be456.js";const r={},u=e(`<h2 id="命令行" tabindex="-1"><a class="header-anchor" href="#命令行" aria-hidden="true">#</a> 命令行</h2><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202312071456443.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>Go中的命令包含了一整套工具链，这些命令涵盖了文档，格式化，代码检查，编译，测试，依赖管理等多个方面，可以说是涉及到了Go开发的方方面面。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>bug         报告漏洞
build       编译包和依赖
clean       清除对象文件
doc         展示源代码中的文档
env         查看Go环境变量信息
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code>tidy</code>命令会清除<code>go.mod</code>中的无用依赖项，也就是没有被引用的依赖项，以及会下载哪些了被引用但是不存在的依赖项。它支持以下参数</p>`,218),w=n("li",null,[n("code",null,"-v"),s("，输出那些被删除的模块依赖")],-1),O=n("li",null,[n("code",null,"-e"),s("，如果过程中发生错误则忽略它继续执行")],-1),q=n("li",null,[n("code",null,"-x"),s("，输出执行过程")],-1),G=n("li",null,[n("code",null,"-go=version"),s("，更新"),n("code",null,"go.mod"),s("文件中的go版本")],-1),$=n("code",null,"-compact=version",-1),C=n("code",null,"go",-1),E={href:"https://stackoverflow.com/questions/71973152/go-mod-tidy-error-message-but-go-1-16-would-select",target:"_blank",rel:"noopener noreferrer"},T=e(`<p>看一个使用例子</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ go mod tidy <span class="token parameter variable">-v</span>
...
<span class="token operator">&lt;</span>/body<span class="token operator">&gt;</span>
<span class="token operator">&lt;</span>/html<span class="token operator">&gt;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="build-1" tabindex="-1"><a class="header-anchor" href="#build-1" aria-hidden="true">#</a> build</h3>`,61),_=n("a",{href:"#%E7%BC%96%E8%AF%91%E6%8E%A7%E5%88%B6"},"build-编译控制",-1),F=n("code",null,"// +build",-1),R=n("code",null,"//go:build",-1),D={href:"https://pkg.go.dev/cmd/go#hdr-Build_constraints",target:"_blank",rel:"noopener noreferrer"},B=e(`<div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token comment">//go:build (linux &amp;&amp; 386) || (darwin &amp;&amp; !cgo)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div>`,56),I={class:"hint-container tip"},H=n("p",{class:"hint-container-title"},"提示",-1),N=n("code",null,"runtime",-1),M={href:"https://github.com/golang/go/blob/master/src/runtime/HACKING.md#runtime-only-compiler-directives",target:"_blank",rel:"noopener noreferrer"},V=n("h3",{id:"",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#","aria-hidden":"true"},"#")],-1);function U(L,j){const i=l("ExternalLinkIcon"),o=l("RouterLink");return c(),d("div",null,[u,n("p",null,[s("本文只是简单的叙述与介绍它们的使用，所有内容参考自官方文档，想要了解更多细节可以前往"),n("a",v,[s("cmd/go"),a(i)]),s("。")]),m,n("p",null,[s("当然也有开源的自建模块代理方案："),n("a",b,[s("goproxy"),a(i)])]),g,n("p",null,[s("这些设置是为了方便在版本更新过程中而出现了不兼容变化时，方便go回退到以前的旧行为，例如在"),k,s("中不再允许"),h,s("这种情况发生，为此，go官方专门记录了"),f,s("，前往"),n("a",x,[s("GODEBUG"),a(i)]),s("了解更多细节。")]),y,n("ul",null,[w,O,q,G,n("li",null,[$,s("，保留从指定的主要Go版本中所需的任何附加校验和，以便成功加载模块图，并且如果该版本的"),C,s("命令从不同模块版本中加载任何已导入的包，将导致tidy出错。通常很少会用到这个参数，一般在版本版本变更时才会出错，可以前往stackoverflow看看这个回答"),n("a",E,[s('go modules - go mod tidy error message: "but go 1.16 would select" - Stack Overflow'),a(i)])])]),T,n("p",null,[S,s("命令是go语言工具链中提供测试功能的命令，这个功能相当的重要，对于一个软件而言，完善的测试的是必不可少的。这里只是简单的介绍下如何使用"),P,s("命令，如果要了解更多测试相关，前往："),a(o,{to:"/essential/senior/120.test.html"},{default:p(()=>[s("测试")]),_:1})]),A,n("p",null,[s("在"),_,s("部分，讲到了用"),F,s("指令来控制编译行为。而"),R,s("指令是在1.17新出的，意在取代先前的指令，不过现在都1.21也还没有取代，估计以后会以共存的方式存在，关于这个新指令，官方文档也有介绍："),n("a",D,[s("build constraints"),a(i)]),s("。它的功能与前者没什么区别，但是语法更加严格，支持布尔表达式，看一个例子")]),B,n("div",I,[H,n("p",null,[s("还有部分指令限制了只能由"),N,s("包使用，外部是无法使用的， 它们会涉及到更深的东西，想要了解可以在"),n("a",M,[s("Runtime-only compiler directives"),a(i)]),s("中看到有关它们的介绍。")])]),V])}const X=t(r,[["render",U],["__file","cmd.html.vue"]]);export{X as default};
