import{_ as i,V as p,W as c,X as s,Y as n,Z as t,a0 as e,F as o}from"./framework-f06be456.js";const l="/images/essential/cgo_1.png",u="/images/essential/cgo_2.png",d={},r=s("h1",{id:"cgo",tabindex:"-1"},[s("a",{class:"header-anchor",href:"#cgo","aria-hidden":"true"},"#"),n(" CGO")],-1),v=s("p",null,"由于go需要GC，对于一些性能要求更高的场景，go可能不太适合处理，c作为传统的系统编程语言性能是非常优秀的，而cgo可以将两者联系起来，相互调用，让go调用c，将性能敏感的任务交给c去完成，go负责处理上层逻辑，cgo同样支持c调用go，不过这种场景比较少见，也不太建议这么做。",-1),k=s("div",{class:"hint-container tip"},[s("p",{class:"hint-container-title"},"提示"),s("p",null,[n("文中代码演示的环境是win10，命令行用的是"),s("code",null,"gitbash"),n("，windows用户建议提前安装好mingw。")])],-1),m={href:"https://go.dev/blog/cgo",target:"_blank",rel:"noopener noreferrer"},b=s("code",null,"cmd/cgo/doc.go",-1),g={href:"https://pkg.go.dev/cmd/cgo",target:"_blank",rel:"noopener noreferrer"},h=e(`<h2 id="代码调用" tabindex="-1"><a class="header-anchor" href="#代码调用" aria-hidden="true">#</a> 代码调用</h2><p>看下面一个例子</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">package</span> main
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,51),f=s("code",null,"1",-1),y=s("code",null,"2",-1),C={href:"https://pkg.go.dev/cmd/cgo#hdr-Passing_pointers",target:"_blank",rel:"noopener noreferrer"},x=e(`<h2 id="类型转换" tabindex="-1"><a class="header-anchor" href="#类型转换" aria-hidden="true">#</a> 类型转换</h2><p>cgo对c与go之间的类型做了一个映射，方便它们在运行时调用。对于c中的类型，在go中导入<code>import &quot;C&quot;</code>之后，大部分情况下可以通过</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>C.typename
...
1 dead
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="指针" tabindex="-1"><a class="header-anchor" href="#指针" aria-hidden="true">#</a> 指针</h3><figure><img src="`+l+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><figure><img src="'+u+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>',95),_=s("code",null,"malloc()",-1),q=s("code",null,"free()",-1),w={href:"https://pkg.go.dev/runtime#Pinner",target:"_blank",rel:"noopener noreferrer"},G=s("code",null,"C.free()",-1),S=e(`<p>如果要在c和go之间传递指针，就需要先将其转为<code>unsafe.Pointer</code>，然后再转换成对应的指针类型，就跟c中的<code>void*</code>一样。看两个例子，第一个是c指针引用go变量的例子，而且还对变量做了修改。</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">package</span> main
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li><p><code>CFLAGS: -I</code>指的是搜索头文件的相对路径，</p></li><li><p><code>-L</code>指的是库搜索路径，<code>\${SRCDIR}</code>代指当前路径的绝对路径，因为它的参数必须是绝对路径</p></li><li><p><code>-l</code>指的是库文件的名称，sum就是<code>sum.dll</code>。</p></li></ul>`,23),P=s("code",null,"CFFLAGS",-1),I=s("code",null,"LDFLAGS",-1),N={href:"https://pkg.go.dev/cmd/cgo#hdr-Using_cgo_with_the_go_command",target:"_blank",rel:"noopener noreferrer"},L=e(`<p>把动态库放到<code>exe</code>的同级目录下</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">ls</span>
...
pure_go_sum: 2 ns/op
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>从结果可以看到，cgo的平均耗时是纯go的二十几倍，倘若执行的不是单纯的两数相加，而是一个比较耗时的任务，cgo的优势会更大一些。除此之外，使用cgo还有以下缺点</p><ol><li>许多go配套工具链将无法使用，比如gotest，pprof，上面的测试例子就不能使用gotest，只能自己手写。</li><li>编译速度变慢，自带的交叉编译也没法用了</li><li>内存安全问题</li><li>依赖问题，如果别人用了你的库，等于也要开启cgo。</li></ol><p>在没有考虑周全之前，不要在项目中引入cgo，对于一些十分复杂的任务，使用cgo确实可以带来好处，但如果只是一些简单的任务，还是老老实实用go吧。</p>`,23);function $(B,D){const a=o("ExternalLinkIcon");return p(),c("div",null,[r,v,k,s("p",null,[n("关于cgo，官方有一个简单的介绍："),s("a",m,[n("C? Go? Cgo! - The Go Programming Language"),t(a)]),n("，如果想要更详细的介绍，可以在标准库"),b,n("中获取更加详细的信息，或者也可以直接看文档"),s("a",g,[n("cgo command - cmd/cgo - Go Packages"),t(a)]),n("，两者内容是完全一样的。")]),h,s("p",null,[n("它有两种检查级别，可以设为"),f,n("，"),y,n("，级别越高检查造成运行时开销越大，可以 前往"),s("a",C,[n("cgo command - passing_pointer"),t(a)]),n("了解细节。")]),x,s("p",null,[n("谈到了指针避不开内存，cgo之间相互调用最大的问题就是两门语言的内存模型并不相同，c语言的内存完全是由开发者手动管理，用"),_,n("分配内存，"),q,n("释放内存，如果不去手动释放，它是绝对不会自己释放掉的，所以c的内存管理是非常稳定的。而go就不一样了，它带有GC，并且Goroutine的栈空间是会动态调整的，当栈空间不足时会进行增长，那么这样一来，内存地址就可能发生了变化，跟上图一样（图画的并不严谨），指针可能就成了c中常见的悬挂指针。即便cgo在大多数情况可以避免内存移动（由"),s("a",w,[n("runtime.Pinner"),t(a)]),n("来固定内存），但go官方也不建议在c中长期引用go的内存。但是反过来，go中的指针引用c中的内存的话，是比较安全的，除非手动调用"),G,n("，否则这块内存是不会被自动释放掉的。")]),S,s("p",null,[P,n("和"),I,n("这两个都是gcc的编译选项，出安全考虑，cgo禁用了一些参数，前往"),s("a",N,[n("cgo command"),t(a)]),n("了解细节。")]),L])}const A=i(d,[["render",$],["__file","135.cgo.html.vue"]]);export{A as default};
