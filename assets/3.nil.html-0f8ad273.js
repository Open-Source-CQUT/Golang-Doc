import{_ as e,V as t,W as p,X as n,Y as s,Z as o,a0 as c,F as i}from"./framework-f06be456.js";const l={},u=c(`<h1 id="nil" tabindex="-1"><a class="header-anchor" href="#nil" aria-hidden="true">#</a> nil</h1><h2 id="引子" tabindex="-1"><a class="header-anchor" href="#引子" aria-hidden="true">#</a> 引子</h2><p>在一次编写代码的过程中，我需要调用<code>Close()</code>方法来关闭多个对象，就像下面的代码一样</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">type</span> A <span class="token keyword">struct</span> <span class="token punctuation">{</span>
	b B
	c C
   	d D
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面这个例子是笔者曾经遇到过的一个bug的简化版，很多初学者刚开始可能都会和我一样犯这种错误，下面就来讲讲到底是怎么个回事。</p><h2 id="接口" tabindex="-1"><a class="header-anchor" href="#接口" aria-hidden="true">#</a> 接口</h2><p>在之前的章节提到过，<code>nil</code>是引用类型的零值，比如切片，map，通道，函数，指针，接口的零值。对于切片，map，通道，函数，可以将它们都看作是指针，都是由指针指向具体的实现。</p><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202312241138669.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>但唯独接口不一样，接口由两个东西组成：类型和值</p><figure><img src="https://public-1308755698.cos.ap-chongqing.myqcloud.com//img/202312241142990.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>当试图对一个变量赋值<code>nil</code>时，会无法通过编译，并且提示如下信息</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>use of untyped nil in assignment
...
&lt;nil&gt;
false
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>结果非常奇怪，明明<code>pa</code>的输出就是<code>nil</code>，但它就是不等于<code>nil</code>。通过反射来看看它到底是什么</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
	<span class="token keyword">var</span> p <span class="token operator">*</span><span class="token builtin">int</span>
	fmt<span class="token punctuation">.</span><span class="token function">Println</span><span class="token punctuation">(</span>p<span class="token punctuation">)</span>
	fmt<span class="token punctuation">.</span><span class="token function">Println</span><span class="token punctuation">(</span>p <span class="token operator">==</span> <span class="token boolean">nil</span><span class="token punctuation">)</span>
	<span class="token keyword">var</span> pa any
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过<code>IsNil()</code>可以直接判断其值是否为<code>nil</code>，这样一来就不会出现上述的问题了。</p><p>在平时使用的过程中，假设函数的返回值是一个接口类型，如果你想返回一个零值，直接返回<code>nil</code>就好了，不要返回任何具体实现的零值，就算它实现了该接口，但它永远也不会等于<code>nil</code>。</p><h2 id="小结" tabindex="-1"><a class="header-anchor" href="#小结" aria-hidden="true">#</a> 小结</h2><p>解决了上面的问题，接下来看看下面这几个例子</p><p>当结构体的接收者为指针接收者时，<code>nil</code>是可用的，看下面一个例子</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">type</span> A <span class="token keyword">struct</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面例子中的这些有关于<code>nil</code>的特性可能会让人比较困惑，<code>nil</code>代表着上面几种类型的零值，也就是默认值，默认值会表现出对应的默认行为，这也正是go的设计者所希望的：让<code>nil</code>变得更有用，而不是直接抛出空指针错误。这一理念同样也体现在标准库中，比如开启一个HTTP服务器可以这样写</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code>http<span class="token punctuation">.</span><span class="token function">ListenAndServe</span><span class="token punctuation">(</span><span class="token string">&quot;:8080&quot;</span><span class="token punctuation">,</span> <span class="token boolean">nil</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>我们可以直接传入一个<code>nil Handler</code>，然后<code>http</code>库就会使用默认的<code>Handler</code>来处理HTTP请求。</p>`,45),d={class:"hint-container tip"},r=n("p",{class:"hint-container-title"},"提示",-1),k={href:"https://www.youtube.com/watch?v=ynoY2xz-F8s&t=56s",target:"_blank",rel:"noopener noreferrer"};function v(m,b){const a=i("ExternalLinkIcon");return t(),p("div",null,[u,n("div",d,[r,n("p",null,[s("感兴趣的可以看看这个视频"),n("a",k,[s("Understanding nil - Gopher Conference 2016"),o(a)]),s("，讲的非常清晰易懂。")])])])}const f=e(l,[["render",v],["__file","3.nil.html.vue"]]);export{f as default};
